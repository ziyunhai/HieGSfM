...
void ConvertDatabaseToGlomap(const colmap::Database& database,
                             const std::unordered_set<image_t>& image_set_ids,
                             ViewGraph& view_graph,
                             std::unordered_map<camera_t, Camera>& cameras,
                             std::unordered_map<image_t, Image>& images) {
  // Add the images
  try {
  auto images_colmap = database.ReadAllImages();
  size_t counter = 0;
  for (auto& image : images_colmap) {
    image_t image_id = image.ImageId();
    ...
    if (image_set_ids.count(image_id) == 0) {
      continue; // zy 图像不存在于cluster set中
    }
    ...
    counter++;
    std::cout << "\r Loading Images " << counter << " / "
              << images_colmap.size() << std::flush;
  }
   std::cout << std::endl;
  ...
  for (auto& [image_id, image] : images) {
    ...
    std::cerr << "捕获到逻辑错误1: " << e.what() << std::endl;
  }
  ...
  for (size_t match_idx = 0; match_idx < all_matches.size(); match_idx++) {
    ...
    if (image_set_ids.count(image_id1) == 0 || image_set_ids.count(image_id2) == 0) {
      continue; // zy 图片不存在，就不需要
    }

    image_pair_count++;
    std::cout << "\r Loading Image Pair " << image_pair_count << " / "
                << all_matches.size() << std::flush;
  ...
  LOG(INFO) << "Pairs read done. " << invalid_count << " / "
    ...
    // 处理异常，例如打印错误信息
    std::cerr << "捕获到逻辑错误: " << e.what() << std::endl;
  }
}
