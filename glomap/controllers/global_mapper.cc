...
void GlobalMapper::Run() {
  ...
  const colmap::Database database_self(database_path_);
  ConvertDatabaseToGlomap(database_self, options_.image_set_ids_, view_graph, cameras, images);

  Solve(database_self, view_graph, cameras, images, tracks);
  ...
  if (largest_component_num == -1) {
    auto reconstruction_idx = reconstruction_manager_->Add();
    std::shared_ptr<colmap::Reconstruction> reconstruction = reconstruction_manager_->Get(reconstruction_idx);
    ConvertGlomapToColmap(cameras, images, tracks, *reconstruction);

    LOG(INFO) << "Extracting colors ...";
    reconstruction->ExtractColorsForAllImages(imagepath_);
  } else {
...
bool GlobalMapper::Solve(const colmap::Database& database,
  ...
  if (!options_.skip_relative_pose_estimation) {
    ...
    RelPoseFilter::FilterTripletConsistency(view_graph, 5.0);
