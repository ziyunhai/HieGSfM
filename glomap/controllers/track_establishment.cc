  ...
  for (auto pair : view_graph_.image_pairs) {
    ...
    for (size_t i = 0; i < inliers.size(); i++) {
      ...
      image_pair_t point_global_id1 =
          ...
          static_cast<image_pair_t>(point1_idx); // 将局部索引point1_idx转化为全局唯一的id
...
size_t TrackEngine::FindTracksForProblem(
  ...
  std::vector<std::pair<size_t, track_t>> track_lengths; // 轨迹长度，轨迹id
  ...
  std::unordered_map<image_t, track_t> tracks_per_camera; // 每个相机分配的轨迹数量
  ...
  for (const auto& [track_length, track_id] : track_lengths) {
    ...
    for (const auto& [image_id, feature_id] : track.observations) {
      if (tracks_per_camera.count(image_id) == 0) continue; // 观测点相机不在相机列表tracks_per_camera中
    ...
    if (image_ids.size() < options_.min_num_view_per_track) continue; // Condition1: 轨迹观测的相机数量<阈值
